...
const GetConferenceCSV = async (_req: Request, res: Response): Controller => {
  const email = TargetEmailForCSV;
  const scope = "Conference Summit";

  try {
    const query = await prisma.conferenceSummit.findMany();
...
export default GetConferenceCSV;
