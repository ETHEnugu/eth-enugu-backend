...
const GetSpeakerCSV = async (_req: Request, res: Response): Controller => {
  const email = TargetEmailForCSV;
  const scope = "Speaker and Mentor";

  try {
    const query = await prisma.speakerApplication.findMany();
